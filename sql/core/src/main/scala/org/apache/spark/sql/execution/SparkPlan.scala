...
abstract class SparkPlan extends QueryPlan[SparkPlan] with Logging with Serializable {
   ...
   */
  def clearBroadcasts(removeFromDriver: Boolean = false): Unit = {
    var hasExecutionPlan = removeFromDriver
    foreach {
      case _: ExecutedCommandExec if !hasExecutionPlan => hasExecutionPlan = true
      case broadcastExchange: BroadcastExchangeExec => broadcastExchange.relationBroadcast match {
        case Some(b) =>
          sparkContext.env.broadcastManager.unbroadcast(b.id, hasExecutionPlan, blocking = false)
        case _ =>
      }
      case _ =>
    }
